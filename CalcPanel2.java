        ...
        //The equals button uses a ScriptEngine to interpret the input.
        {
            ...
            else if(event.getSource() == buttonEquals)
            {
                if(oldInput.equals("1 - 100"))
                {
                    ...
                    turns = 0;
                    game = true;
                    labelAnswer.setText("I'm thinking of a number from 1 to 100...");
                    labelInput.setText("");
                }
                else if(game && ! oldInput.equals(""))
                {
                    if(Integer.parseInt(oldInput) > 100 || Integer.parseInt(oldInput) == 0)
                    {
                        labelAnswer.setText("Your guess must be from 1 to 100");
                        labelInput.setText("");
                    }
                    else
                    {
                        turns ++;
                        if(Integer.parseInt(oldInput) == num)
                        {
                            labelAnswer.setText("Correct!");
                            JOptionPane.showMessageDialog(null, "You guessed " + num + " in " + turns + " tries!\n" + (turns <= 4 ? "Total luck, of course." : (turns <= 7 ? "Great job!" : "It should never take that many.")));
                        ...
                        else if(Integer.parseInt(oldInput) > num)
                        {
                ...
                else if(! oldInput.equals("") && oldInput.charAt(oldInput.length() - 1) != ' ' && oldInput.charAt(oldInput.length() - 1) != '-' && ! oldInput.equals("."))
                {
